        ...
        this.progBaseService.getAllProgramacionBaseByEm(this.emID,0)
            .subscribe(
                data=>{
                    ...
                    if(arrProgrBases.length!=0){
                        let arrprog=[];
                        this._PrBaId=arrProgrBases[arrProgrBases.length-1].PrBaId;  //buscando por la ultima programacionbase creada
                        this.PrBaId=arrProgrBases[arrProgrBases.length-1].PrBaId; //buscando por la ultima programacionbase creada
                        console.log(this.PrBaId);
                        this.progService.getallprogramaciondetallebyprbafecha(this.PrBaId,this.fechaAsTarjUno)
                            .subscribe(
                                data=>{
                                    arrprog=data;
                                    if(arrprog.length>0 ){
                    ...
                    }else if(arrProgrBases.length==0){
...
/* dia con programacion */
                ...
                data => {
                            ...
                            if(arrCuadro.length!=0 && arrCuadro.length>0){                                         
                                ...
                                if(sigVuelta==1){ //si fuera la ultima vuelta
                                    ...
                                    this.concatenarRegDiarioProgxFecha(this.mgprogDetalle(this.progDetalle),arrCuadro);
                ...
                this.progService.getallprogramaciondetallebyprbafecha(PrBaId, fecha).subscribe(
                    data => {  
                        ...
                        if(this.progDetalle.length>0 ){
                            ...
                            this.tcontrolservice.getallregistrovueltasdiariasbyemprbafe(this.emID, PrBaId,this.fechaAsTarjDos).subscribe(
                                data=>{
                                    console.log(data);
                                    this.concatenarRegDiarioProgxFecha(this.mgprogDetalle(this.progDetalle), data);
                                },error=>{}
                ...
                this.tcontrolservice.asignarTarjetaControl(_tarjeta).subscribe(
                    data => {   
                                if(data==true){
                                    ...
                                    this.progBaseService.getAllProgramacionBaseByEm(this.emID,0).subscribe(
                                        data=>{
                                            this.actualizargetallregistrovueltasdiariasbyemprfeINDIVIDUAL(this.emID,data[data.length-1].PrBaId,this.fechaAsTarjDos, sigVuelta);
                                        },
                                    );
                                }                               
                            }, 
        ...
        //MOSTRAR RESULTADO EN ASIGNAR TABLAS - NRO VUELTAS POR EL DIA
            ...
            buscarDatosTarjetaXNroVuelta(arrCuadroXFecha=[], ReDiDeNroVuelta:number){
                ...
                if(arr.length!=0){
                    ...
                    this.PrBaId=arr[0].PrBaId
                    this._PrBaId=arr[0].PrBaId
    ...
    // FUNCIONES BUSQUEDA
        ...
        buscarHoraSalidaAnterior(placa:string){
            ...
            this.progBaseService.getAllProgramacionBaseByEm(this.emID,0).subscribe(
                data=>{
                    this.tcontrolservice.getallregistrovueltasdiariasbyemprbafe(this.emID,data[data.length-1].PrBaId,this.fechaAsTarjDos).subscribe(
                        data => {
                                    arrCuadro=data;    
                                    if(arrCuadro.length!=0 && arrCuadro.length>0){
                        ...
                        error=> {alert('Error en el cuadro de salidas dia con programacion, vuelva a intentarlo');},
                        ()   => {}
    ...
    /* FUNCIONES DE COMBOBOX */
            ...
            funcCboTipoTarjeta(){
                ...
                //individual
                    ...
                    .subscribe(
                        data=>{
                            console.log(data[data.length-1].PrBaId);
                            this.getallprogramaciondetallebyprbafecha(data[data.length-1].PrBaId,this.fechaAsTarjUno, this.nroTarjetas); // tabla programacion
                    });
                }
            }
